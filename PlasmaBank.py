#for calling external commands and getting their outputs using python
...
def executeQuery(query):
	cur.execute(query)
	con.commit()  #ran successfully

#function to handle exceptions
def excepting(e):
	con.rollback()    #everything untill prev commit will be erased
	print("Failed to insert/delete/update/find database")
	print (">>>>>>>>>>>>>", e)
	return

#operation : Insert(donor_id, date_of_donation) to DONORS table
#Donor_id is not present in DONOR_INFO then cannot insert
def addDonor():
	try :
		row={}
...
#helper function for adding donor
#operation : Insert(donor_id,blood_type,phone_no,dOB,age,sex,fname,lname,address) to DONOR_INFO
...
#operation : Insert(plasma_bag_number,blood_type,blood_amount,platelets_count) to BLOOD
#cannot be inserted if plasma_bag_number is absent in blood cost
def addBlood():
	try:
...
#we can only insert if there is rec_id already in receives(emp_id, rec_id) table
# operation : Insert(rec_id,blood_type,quantity_needed,date_of_request,fname,lname,dOB,sex,age,address) to RECIPIENT
def addRecipient():
	try:
		row = {}
...
#operation : INSERT(emp_id,fname,supervisor, address1,phone_no,salary) to STAFF
def hireStaff():
	try:
		row = {}
...
#if plasma_bag_number exists delete it from BLOOD
#ie the blood has been used
def deleteBlood():
...
#if plasma_bag_number exists in blood then UPDATE BLOOD cost
#Update cost in BLOOD_COST to new_cost
def UpdateBloodCost():
...
#retrieve the sum of plasma available
def TotalBlood():
	try :
...
#retrieve the sum of plasma available of particular blood_type
...
#list of all bags with blood_type, blood_amount, platelets_count
def ViewBlood():
	try :
...
#???
#Ambigous: blood cost has a FOREIGN KEY plasma_bag_number that needs to be there???????
#???
def AddBloodCost():
	try :
		row={}
...
#get list of all employees under the supervisor
def Supervisors():
	try :
...
#only reciepients can pay
#FOREIGN KEY is rec_id from receipients
def addPaymentTnx():
	try:
		row = {}
...
# Global
while(1):
	#shell = True can pass all argument in a string
	tmp = sp.call('cls',shell=True)   #replace clear -> cls
	user = input("Username: ")
	...
	try:
		...
		#con.cursor() create a cursor object
		#we can call .execute method on this object to perform a query
